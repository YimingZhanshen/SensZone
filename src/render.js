(function (SZ) {
  ...
  function updateBasis() {
    const a = cam.az * SZ.math.RAD,
      e = cam.el * SZ.math.RAD;
    const ca = Math.cos(a),
      sa = Math.sin(a),
      ce = Math.cos(e),
      se = Math.sin(e);
  ...
  function drawFlashes(flashes, now) {
    for (const f of flashes) {
      ...
      ctx.strokeStyle =
        kind === 'hit' ? COL.flashHit : kind === 'deny' ? COL.flashDeny : COL.flashMiss;
      ctx.stroke();
      ctx.globalAlpha = 1;
    }
  }
