(function (SZ) {
  ...
  function log2Cm(cm) { return Math.log2(36 / cm); }
  function cmFromLog2(x) { return 36 / Math.pow(2, x); }
  ...
  function setupAxes(ctx, box, xDomain, yDomain) {
    ...
    const mapX = cm => plot.x + (log2Cm(cm) - log2Cm(xDomain[0])) / (log2Cm(xDomain[1]) - log2Cm(xDomain[0])) * plot.w;
  ...
  function drawMtChart(canvas, report) {
    ...
    const yDomain = [Math.min(250, Math.min.apply(null, mts) * 0.7), Math.max.apply(null, mts) * 1.35];

    const box = { x: 0, y: 0, w: cssW, h: cssH };
    const { plot, mapX, mapY } = setupAxes(ctx, box, xDomain, yDomain);
    drawFrame(ctx, plot, [10, 15, 20, 30, 45, 60, 80, 120], niceTicks(yDomain[0], yDomain[1], 5), mapX, mapY, { x: 'cm/360', y: 'MT* (ms, ID=' + (report.idRef != null ? report.idRef : 3) + ')' });

    const paperL = mapX(20), paperR = mapX(80);
    ...
    for (const name of ['low', 'mid', 'high']) {
      ...
      for (let cm = xDomain[0]; cm <= xDomain[1]; cm *= 1.05) {
        ...
        if (v < yDomain[0] || v > yDomain[1]) { started = false; continue; }
        const x = mapX(cm), y = mapY(v);
      ...
      ctx.stroke();
      if (fit.ci) {
        const l = mapX(fit.ci[0]), r = mapX(fit.ci[1]);
    ...
    for (const c of used) {
      const x = mapX(c.cm360), y = mapY(mtOf(c));
      ctx.beginPath();
      ctx.arc(x, y, 3.5, 0, Math.PI * 2);
      ctx.fill();
      const se = c.adjSE != null ? c.adjSE : (c.sdMT != null ? c.sdMT / Math.sqrt(Math.max(1, c.n * c.hitRate)) : null);
      if (se != null) {
        ctx.beginPath();
        ctx.moveTo(x, mapY(mtOf(c) - se)); ctx.lineTo(x, mapY(mtOf(c) + se));
    ...
    if (ax >= plot.x && ax <= plot.x + plot.w) {
      ctx.beginPath();
      ctx.moveTo(ax, plot.y); ctx.lineTo(ax, plot.y + plot.h);
      ctx.setLineDash([5, 4]);
  ...
  function drawErrPanel(ctx, box, conds) {
    ...
    const plot = { x: box.x + padL, y: box.y + padT, w: box.w - padL - padR, h: box.h - padT - padB };
    const maxAbs = Math.max(0.5, Math.max.apply(null, conds.map(c => Math.abs(c.meanSignedErr))) * 1.3);
    const mapY = v => plot.y + plot.h / 2 - v / maxAbs * (plot.h / 2);
    ctx.strokeStyle = 'rgba(120,140,160,0.35)';
    ctx.strokeRect(plot.x, plot.y, plot.w, plot.h);
    ctx.beginPath();
    ctx.moveTo(plot.x, mapY(0)); ctx.lineTo(plot.x + plot.w, mapY(0));
    ctx.strokeStyle = 'rgba(200,220,240,0.5)';
    ctx.stroke();
    const bw = plot.w / conds.length * 0.5;
    conds.forEach((c, i) => {
      const cx = plot.x + (i + 0.5) / conds.length * plot.w;
      const y0 = mapY(0), y1 = mapY(c.meanSignedErr);
  ...
  function drawSubPanel(ctx, box, conds) {
    ...
    const xMin = Math.min.apply(null, xs) - 0.3, xMax = Math.max.apply(null, xs) + 0.3;
    const mapX = x => plot.x + (x - xMin) / (xMax - xMin) * plot.w;
    const mapY = v => plot.y + plot.h - (v - yMin) / (yMax - yMin) * plot.h;
    ctx.beginPath();
    usable.forEach((c, i) => {
      const x = mapX(xs[i]), y = mapY(c.meanSubmov);
  ...
  function drawRmsPanel(ctx, box, used, tr, report) {
    ...
    const { plot, mapX, mapY } = setupAxes(ctx, box, xDomain, yDomain);
    drawFrame(ctx, plot, [10, 20, 45, 80, 150], niceTicks(yDomain[0], yDomain[1], 4), mapX, mapY, { x: 'cm/360', y: 'RMS err (\u00b0)' });
    ...
    if (fit && tr.global.method.indexOf('quadratic') === 0) {
      ...
      ctx.stroke();
      if (tr.global.ci) {
        const l = mapX(tr.global.ci[0]), r = mapX(tr.global.ci[1]);
        ctx.fillStyle = '#b388ff1c';
        ctx.fillRect(Math.min(l, r), plot.y, Math.abs(r - l), plot.h);
      }
    }
    if (tr.plateau) {
      const l = mapX(tr.plateau.lo), r = mapX(tr.plateau.hi);
    ...
    ctx.fillStyle = '#b388ff';
    for (const c of used) {
      const x = mapX(c.cm360), y = mapY(c.rms);
      ctx.beginPath();
      ctx.arc(x, y, 3.5, 0, Math.PI * 2);
      ctx.fill();
      if (c.se != null) {
        ctx.beginPath();
        ctx.moveTo(x, mapY(c.rms - c.se)); ctx.lineTo(x, mapY(c.rms + c.se));
        ctx.strokeStyle = 'rgba(245,247,250,0.5)';
        ctx.lineWidth = 1.5;
        ctx.stroke();
      }
  ...
  function drawLagPanel(ctx, box, used) {
    ...
    const padL = 44, padR = 10, padT = 26, padB = 34;
    const plot = { x: box.x + padL, y: box.y + padT, w: box.w - padL - padR, h: box.h - padT - padB };
    ctx.strokeStyle = 'rgba(120,140,160,0.35)';
    ctx.lineWidth = 1;
    ctx.strokeRect(plot.x, plot.y, plot.w, plot.h);
  ...
  function niceTicks(lo, hi, n) {
    ...
    for (const c of candidates) {
      if (c * mag >= step0) { step = c * mag; break; }
    }
    const ticks = [];
  ...
  SZ.charts = { drawMtChart, drawTrackChart, drawDiagChart, niceTicks, BUCKET_COLORS };
